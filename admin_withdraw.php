...
$qury_wallet = "SELECT *FROM withdraw Where status_withdraw = '0' ";
$sql_wallet = mysqli_query($conn,$qury_wallet);
while($show_sql_wallet = mysqli_fetch_array($sql_wallet))
{

        if($show_sql_wallet['status_withdraw']==0)
        {
            $new_status = "รอยืนยัน";
        }
...
?>
        <td><?php echo $show_sql_wallet['withdraw_id']; ?></td>
        <td><?php echo $show_sql_wallet['payment_type_withdraw']; ?></td>
        <td><?php echo $show_sql_wallet['bank_name_withdraw']; ?></td>
        <td><?php echo $show_sql_wallet['bank_number_withdraw']; ?></td>
        <td><?php echo number_format($show_sql_wallet['adwoo_coin_withdraw'], 2);?></td>
        <td><?php echo $show_sql_wallet['withdraw_time'];  ?></td>
        <td><?php echo "$new_status";   ?></td>
        <td><form action="admin_status_db.php" method="post" ><button type="submit" class="button" name = "update_Withdraw" value = "<?php echo $show_sql_wallet['withdraw_id']; ?>" ><span>ยืนยัน</span></button>                     </form></td>
      </tr>
      <tr>
<?php 
